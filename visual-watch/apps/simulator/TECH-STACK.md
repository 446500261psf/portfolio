...
- **双层结构**（光场紧贴玻璃）：
  - **贴玻璃显示层（解析）**：在光线入射内壁点直接采样光场一次，
    `frontness²` 只作用于正面 —— UI 像手机屏一样浮在玻璃正下方，
    零步进噪声、清晰锐利；swirl 不作用于此层，UI 稳定不晃
  - **Pool 体积层（raymarch）**：壳层密度 `smoothstep(0.35,1,ρ)²`
    偏向边缘与背部（前壁权重 ×0.25），加 0.07 稀薄填充 ——
    光沿边缘、背部流动，正面不再有雾感
- **边缘流动**：体积层光场绕潭心随时间缓慢摆动（±0.32rad，14s 周期），
  越贴内壁摆幅越大
